...
class YoloV1Loss(nn.Module):
    ...
    def forward(
        ...
        # shape: (batch, B, S, S)
        ious = self._get_ious(
            preds.detach(), targets, self.image_height, self.image_width
        )
    ...
    def _get_confidence_loss(self, preds, ious, responsibility):
        ...
        for i in range(self.num_boxes):
            ...
            c_loss = responsibility[..., i] * c_loss
            c_loss = c_loss.sum(dim=(1, 2)).mean(dim=0)
    ...
    def _get_ious(self, preds, targets, image_height, image_width) -> torch.Tensor:
        """
        ...
        ious = []
        for i in range(self.num_boxes):
            iou = self.custom_ious(bboxes[i], bboxes[-1])  # shape (batch, S, S)
            ious.append(iou)
    ...
    def custom_ious(self, boxes1, boxes2) -> torch.Tensor:
        ...
        side_x = self._max(zeros, x2 - x1 + 1)
        side_y = self._max(zeros, y2 - y1 + 1)
