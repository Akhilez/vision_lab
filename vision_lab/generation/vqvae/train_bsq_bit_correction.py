...
class BSQantizer(nn.Module):
    def __init__(self, num_emb, num_bits, flip_prob=0.3):
        super().__init__()
        ...
        self.bit_correction = nn.Sequential(
            # TODO: Maybe add an attention layer here instead of many convs
            nn.Conv2d(num_bits, num_emb, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(num_emb, num_emb, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(num_emb, num_emb, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
            nn.Conv2d(num_emb, num_bits, kernel_size=3, stride=1, padding=1),
        )
    ...
    @staticmethod
    def flip_signs(x, p):
        p = (torch.rand((1,)) * p).item()  # Sample from 0 to p
        # Perturb to flip bits with probability flip_prob
        mask = (torch.rand_like(x) > p) * 2 - 1  # Convert to -1 and 1
        x_flipped = x * mask
        # print((x_flipped == x).float().mean(), " same")
        return x_flipped
    ...
    def forward(self, x):
        ...
        if self.training:
            true_quantized = self.quantize(x).detach()
            x = self.flip_signs(x, self.flip_prob)

        u = self.quantize(x)
        u_corrected = self.quantize(self.bit_correction(u.detach()))

        loss_entropy = self.get_entropy_loss(u)
        loss_correction = torch.tensor(0.0)

        if self.training:
            loss_correction = F.binary_cross_entropy(u_corrected.view(b, -1), true_quantized.view(b, -1))
            # loss_correction = F.mse_loss(u_corrected, true_quantized)
        ...
        return x, u, (loss_entropy, loss_correction)
...
class BSQVAE(nn.Module):
    ...
    def forward(self, x):
        x_orig = x
        x = self.encoder(x)
        x, u, (loss_entropy, loss_correction) = self.quantizer(x)
        x = self.decoder(x)

        loss_reconstruction = F.mse_loss(x, x_orig)

        return x, (loss_reconstruction, loss_entropy, loss_correction)
...
def evaluate(val_loader, model, device, epoch):
    ...
    loss_reconstruction, loss_entropy, loss_correction = losses
    loss = loss_reconstruction + loss_entropy + loss_correction
    print(f"Validation loss: {loss.item()} (Recon: {loss_reconstruction.item()}, Entropy: {loss_entropy.item()}), Correction: {loss_correction.item()}")
...
def train():
    ...
    while step < max_steps:
        ...
        for x, _ in tqdm(train_loader, total=len(train_loader)):
            ...
            if step >= max_steps:
                break
        print(f"Epoch: {epoch}, global step {step}, Recon: {loss_recon_agg / len(train_loader)}, Entropy: {loss_entropy_agg / len(train_loader)}, Correction: {loss_correction / len(train_loader)}")
